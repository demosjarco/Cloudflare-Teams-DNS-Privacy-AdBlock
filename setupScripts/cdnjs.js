...
class CDNJS {
	...
	writeHTML(libraryName, version, libraryType, filename, sri) {
		...
		switch (libraryType) {
			case 'css':
				replacedHTML = originalHtml.replace(commentPatternPreload, `\n\t<link rel="preload" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" as="style" crossorigin />`);
				replacedHTML = originalHtml.replace(commentPattern, `\n\t<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" integrity="${sri}" crossorigin="anonymous" referrerpolicy="no-referrer" />`);
				break;
			case 'js':
