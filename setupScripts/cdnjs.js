...
class CDNJS {
	...
	writeHTML(libraryName, version, libraryType, filename, sri) {
		...
		switch (libraryType) {
			case 'css':
				replacedHTML = replacedHTML.replace(commentPatternPreload, `<link rel="preload" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" as="style" crossorigin />`);
				...
				break;
			case 'js':
				replacedHTML = replacedHTML.replace(commentPatternPreload, `<link rel="preload" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" as="script" crossorigin />`);
