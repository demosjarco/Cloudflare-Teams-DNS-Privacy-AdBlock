...
export class CDNJS {
	...
	private writeHTML(libraryName: string, version: string, libraryType: 'js' | 'css', filename: string, sri: string) {
		...
		switch (libraryType) {
			case 'css':
				html = html.replace(commentPatternPreload, `<link rel="preload" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" integrity="${sri}" as="style" crossorigin="anonymous" referrerpolicy="no-referrer" />`);
				html = html.replace(commentPattern, `<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" integrity="${sri}" crossorigin="anonymous" referrerpolicy="no-referrer" />`);
				break;
			case 'js':
				html = html.replace(commentPatternPreload, `<link rel="preload" href="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" integrity="${sri}" as="script" crossorigin="anonymous" referrerpolicy="no-referrer" />`);
				html = html.replace(commentPattern, `<script src="https://cdnjs.cloudflare.com/ajax/libs/${libraryName}/${version}/${filename}" integrity="${sri}" crossorigin="anonymous" referrerpolicy="no-referrer" defer></script>`);
				break;
		}
